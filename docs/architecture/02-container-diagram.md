...
- **PostgreSQL is the single source of truth for domain state:** Projects, API keys, templates, recipients, notifications, delivery attempts, engagement events, delivery constraints, blackout dates, restricted windows, webhook registrations, webhook events, model version metadata, and training run metadata all live in Postgres. Redis holds only ephemeral state with defined TTLs. This means Redis can be flushed or replaced without data loss, only active sessions, in-flight rate limit windows, revocation cache entries, and recent idempotency keys are affected.
