...
- **Notification:** Represents a request accepted by Tiber to deliver a message to a recipient. A notification is immutable once accepted and progresses through scheduling, delivery, retries, and completion. A notification may carry client-supplied `context`: a flat string-to-string payload of client-side facts (user segment, campaign id, feature flags) consumed by the ML layer for training or inference. Context is machine-facing feature data, never delivery data — it must never contain recipient addresses or message content, which have their own fields — and it must carry only what Tiber cannot derive itself (e.g. recipient timezone is legitimate; subscription status is not, since consent state already lives on the Recipient). Context is validated at intake and carried on the in-memory entity, but deliberately not persisted on the notification row or on engagement events: its storage home is a separate per-notification store for ML features (features attach to the prediction, not the outcome), deferred to the ML phase — so no training data accrues until that store exists, a known and accepted cost.
